...
@section('title', ($item->name ?? 'Product Details') . ' | PeytonGhalib')
@section('meta_description', \Illuminate\Support\Str::limit(strip_tags($item->description ?? 'Shop ' . ($item->name ?? 'this product') . ' at PeytonGhalib. Quality furniture and home decor at unbeatable prices with fast delivery.'), 155))
                ...
                <div class="py-4 sm:py-6 border-b border-bdr-clr dark:border-bdr-clr-drk" data-aos="fade-up" data-aos-delay="300">
                    <div class="flex gap-x-12 gap-y-3 flex-wrap">
                        ...
                        <h6 class="leading-none font-medium text-sm">Category :
                            <a href="{{ url('/shop?category=' . $item->category->slug) }}" class="font-normal text-gray-500 dark:text-white/60 hover:text-primary duration-200">{{ $item->category->name }}</a>
                        </h6>
                        @endif
                        @if($item->tag)
                    ...
                    <div class="flex flex-wrap gap-[10px] md:gap-[15px] mt-5 md:mt-6">
                        @if($item->category)
                            <a class="btn btn-theme-outline btn-xs"
                               href="{{ url('/shop?category=' . $item->category->slug) }}"
                ...
                <div class="pt-4 sm:pt-6" data-aos="fade-up" data-aos-delay="500">
                    <div class="flex items-center gap-6">
                        ...
                        <div class="flex gap-6">
                            <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(route('product-details', $item->slug)) }}" target="_blank" rel="noopener noreferrer nofollow" aria-label="Share on Facebook" class="text-paragraph duration-300 dark:text-white hover:text-primary dark:hover:text-primary">
                            ...
                            <a href="https://twitter.com/intent/tweet?text={{ urlencode($item->name . ' — PeytonGhalib') }}&url={{ urlencode(route('product-details', $item->slug)) }}" target="_blank" rel="noopener noreferrer nofollow" aria-label="Share on Twitter" class="text-paragraph duration-300 dark:text-white hover:text-primary dark:hover:text-primary">
                            ...
                            <a href="https://www.instagram.com/peytonghalib" target="_blank" rel="noopener noreferrer nofollow" aria-label="Follow on Instagram" class="text-paragraph duration-300 dark:text-white hover:text-primary dark:hover:text-primary">
...
@push('scripts')
@php
    $jsonLdImage = !empty($item->image)
        ? (str_starts_with($item->image, 'assets/') ? asset($item->image) : \Storage::url($item->image))
        : asset('assets/img/gallery/product-detls/product-01.jpg');
    $jsonLdPrice = number_format($item->sale_price ?? $item->price, 2);
@endphp
<script type="application/ld+json">
{
  ...
  "description": {{ json_encode(strip_tags($item->description ?? '')) }},
  @if($item->sku)"sku": {{ json_encode($item->sku) }},@endif
  "brand": {
    "@type": "Brand",
    "name": "PeytonGhalib"
  },
  "offers": {
    "@type": "Offer",
    "url": {{ json_encode(route('product-details', $item->slug)) }},
    "priceCurrency": "USD",
    "price": {{ json_encode($jsonLdPrice) }},
  ...
  ,"aggregateRating": {
    ...
    "ratingValue": "{{ number_format($item->avgRating(), 1) }}",
