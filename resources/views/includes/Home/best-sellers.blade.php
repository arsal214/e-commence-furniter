@forelse ($bestSellers as $product)
    <div class="group">
        <div class="relative overflow-hidden">
            <a href="{{ route('product-details', $product->slug) }}">
                @if ($product->image)
                    @if (str_starts_with($product->image, 'assets/'))
                        <img loading="lazy" decoding="async" class="w-full h-56 object-cover transform group-hover:scale-110 duration-300" src="{{ asset($product->image) }}" alt="{{ $product->name }}">
                    @else
                        <img loading="lazy" decoding="async" class="w-full h-56 object-cover transform group-hover:scale-110 duration-300" src="{{ Storage::url($product->image) }}" alt="{{ $product->name }}">
                    @endif
                @else
            ...
            <div class="pg-card-actions absolute z-10 bottom-3 right-3 flex flex-col items-end gap-2">
                <button type="button" class="pg-ico-btn wishlist-toggle-btn"
